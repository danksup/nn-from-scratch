...
class LayerNorm:
    ...
    def forward(self, x:np.ndarray):
        ...
        self.x_norm = (x-mean) / np.sqrt(self.var + self.epsilon)
        self.output = self.gamma * self.x_norm + self.beta

        return self.output
    def backward(self,gradient):
        self.d_beta =  np.sum(gradient, axis=(0,1))
        self.d_gamma = np.sum(gradient*self.x_norm, axis=(0,1))
        self.dx_norm = gradient * self.gamma
        inv_std = 1.0 / np.sqrt(self.var + self.epsilon)
        dx = inv_std * (self.dx_norm - np.mean(self.dx_norm, axis=-1,keepdims=True) - self.x_norm * np.mean(self.dx_norm * self.x_norm, axis=-1,keepdims=True))
