...
class TransformerBlock:
    def __init__(self,embed_dim=None,ff_dim=None, n_heads=None) -> None:
        if embed_dim is not None and ff_dim is not None and n_heads is not None:
            ...
            self.scale_1persqrtdim = nx.float_32(1.0/nx.sqrt(embed_dim))
    ...
    def _forward(x, embed_dim, n_heads, head_dim,freqs, Wqkv, Wo, Wcombined, hidden_width, Wout, epsilon:float, gamma1, gamma2, p:float, is_training:bool):
        ...
        attn_out, caches_attn = AttentionLayer._forward(rmsnorm1_out, embed_dim, n_heads, head_dim, freqs, Wqkv, Wo,)
    ...
    def forward(self,x, is_training:bool):
        ...
        ff_out ,masks, caches = self._forward(x, self.embed_dim, self.n_heads, self.head_dim,self.freqs, Wqkv, Wo, Wcombined, self.hidden_width, Wout, epsilon, gamma1, gamma2, 0.1, is_training)
    ...
    def backward(self, gradient: tuple, masks:tuple, all_caches:tuple) -> Any:
